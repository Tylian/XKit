	...
	do: function() {
		...
		$(posts).each(function() {
			...
			if (m_post.owner !== XKit.extensions.limit_people.last_url) {
				if (XKit.extensions.limit_people.last_grouped && !$(this).hasClass("xkit-limit-people-grouped-hidden") && (XKit.extensions.limit_people.last_count - 1) > after_v - 1) {
					console.log("** LIMIT PEOPLE: Closing the previous group.... [" + XKit.extensions.limit_people.last_url+ "]");
					$(".xkit-limit-people-group-" + XKit.extensions.limit_people.last_url + "-" + XKit.extensions.limit_people.last_group_id + "-container").not(".xkit-limit-people-grouped-hidden").last().after("<li class=\"post_container\"><div data-to-get=\"" +  (".xkit-limit-people-group-" + XKit.extensions.limit_people.last_url + "-" + XKit.extensions.limit_people.last_group_id)  + "\" data-group-id=\"" + XKit.extensions.limit_people.last_group_id + "\" class=\"xkit-limit-people-grouped-warning xkit-limit-people-opener\">Show " + (XKit.extensions.limit_people.last_count - after_v) + " more by " + XKit.extensions.limit_people.last_url + "</div></div>");	
				}
			...
			if (XKit.extensions.limit_people.last_count >= after_v) {
				$(this).addClass("xkit-limit-people-grouped-hidden");
