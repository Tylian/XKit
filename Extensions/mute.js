...
//* DETAILS This extension allows you to hide certain types of posts from a user while still seeing their other posts. Useful if a blogger has nice posts but a bad personality. Please note that you'll need to re-mute them if a user changes their URL. **//
...
XKit.extensions.mute = new Object({
	...
	run: function() {
		...
		var do_it_yourself = false;
                ...
                if (do_it_yourself) {
                	$(document).on('click','.tumblelog_menu_btn', XKit.extensions.mute.add_links_wo_usermenus);
                }
	...
	should_be_removed: function(type, username) {
	...
	do_posts: function(rethink) {
		...
		if (rethink === true) {
			...
			$(".post.xmute-done").removeClass("xmute-done");
		...
		$(posts).each(function() {
			...
			if (XKit.extensions.mute.should_be_removed(m_post.type, m_post.owner) === true) {
				update_rects = true;
	...
	show_window: function(ud, from_user_menu) {
		...
		$("#xkit-mute-save-changes").click(function() {
			...
			if ( $(".xkit-mute-option.selected").length <= 0) { alert("Please select the post types to block."); return; }
