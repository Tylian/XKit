...
XKit.extensions.tag_replacer = new Object({
	...
	run: function() {
		...
		xf_html = '<ul class="controls_section" id="tag_replacer_ul">' + 
			...
			'<li class="no_push"><a data-url="' + XKit.interface.where().user_url + '" href="#" onclick="return false;" id="tag_replacer_button">' +
				'<div class="hide_overflow">Replace a tag<span class="sub_control link_arrow icon_right icon_arrow_carrot_right"></span></div>' +
			'</a></li></ul>';
		...
		$("#tag_replacer_button").click(function() {
			...
			XKit.extensions.tag_replacer.show($(this).attr('data-url'));
	...
	replace_next: function() {
		...
		m_post_obj.owner = XKit.extensions.tag_replacer.url;
		...
		if (XKit.extensions.tag_replacer.p_array_index >= XKit.extensions.tag_replacer.p_array.length) {
			if (XKit.extensions.tag_replacer.success_count === 0) {
				...
				XKit.window.show("Thank you, come again!","Tag Replacer replaced tags of " + XKit.extensions.tag_replacer.success_count + " posts.","info", "<div class=\"xkit-button default\" id=\"xkit-close-message\">OK</div>");	
			}
			return;
		}
		...
		XKit.interface.fetch(m_post_obj, function(data) {
			...
			XKit.interface.edit(m_post_object, function(data) {
				...
				if (data.error === false && data.data.errors === false) {
					...
					XKit.extensions.tag_replacer.p_array_index++;
					setTimeout(function() { XKit.extensions.tag_replacer.replace_next(); }, 1500);
						
				} else {
					
					XKit.extensions.tag_replacer.show_error("<b>Unable to edit post.</b><br/>Please try again later.<br/><br/>Error Code: TGR-835");
	...
	next: function() {
		...
		var m_url = "http://" + XKit.extensions.tag_replacer.url + ".tumblr.com/tagged/" + XKit.extensions.tag_replacer.t_replace + "/page/" + XKit.extensions.tag_replacer.page + "/json";
		
		if (XKit.extensions.tag_replacer.page === 1) {
			m_url = "http://" + XKit.extensions.tag_replacer.url + ".tumblr.com/tagged/" + XKit.extensions.tag_replacer.t_replace + "/json";
		}
