	...
	load_tags_no_api: function() {
		...
		var m_post_id = XKit.extensions.replyviewer.post_id;
    		...
    		$.ajax({
    			...
    			XKit.window.close();
    			XKit.window.show("Unable to fetch required data", "ReplyViewer could not get the required data from Tumblr servers. Please try again later or <a href=\"http://xkit-extension.tumblr.com/ask/\">file a bug report</a> by going to the XKit Blog.","error","<div class=\"xkit-button default\" id=\"xkit-close-message\">OK</div>");
    			...
    			if (m_post_id !== XKit.extensions.replyviewer.post_id || m_init_id !== XKit.extensions.replyviewer.init_id) { 
      			...
      			if (next_note > 0) {
      				...
      				if (XKit.extensions.replyviewer.found_count == 0) {
      					$("#replyviewer-loading").html("No posts with tags found.");
      				}
